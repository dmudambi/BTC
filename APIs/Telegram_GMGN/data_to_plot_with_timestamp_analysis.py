...
# Time Intervals
CHECK_INTERVAL = 100        # Time between token checks (seconds)
...
def create_token_plot(df: pd.DataFrame, timeframe: str, token_name: str, token_info: dict) -> go.Figure:
    ...
    NOTIFICATION_COLORS = {
        '2x': '#9400D3',  # Neon deep purple for 2x increase
        'no_2x': '#FF4500'  # Different color for no 2x increase
    }
            ...
            try:
                ...
                for token in df['token'].unique():
                    try:
                        ...
                        max_price_after_notification = token_df['high'].max()
                        if max_price_after_notification < 2 * notification_price:
                            continue  # Skip saving if no 2x increase
