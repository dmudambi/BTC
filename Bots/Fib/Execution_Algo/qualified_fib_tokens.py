...
def plot_price_and_fib_levels(imported_ohlcv_data, fib_levels, initial_timeframe='15m', 
    ...
    for token_address, token_data in imported_ohlcv_data.items():
        try:
            ...
            # If all conditions pass, add token to list
            logging.info(f"Token {token_address} passed all checks!")
            collect_tokens.append({'token_address': token_address})
            
            # Plotting (optional)
            if display_plots:
                plt.figure(figsize=(12, 6))
                plt.plot(df['close'], label='Close Price')
                plt.axhline(y=ath_close, color='r', linestyle='--', label=f'ATH ({ath_close:.4f})')
                for level, price in fib_price_levels.items():
                    plt.axhline(y=price, linestyle='--', label=f'Fib {level} ({price:.4f})')
                plt.title(f'Token: {token_address} - Price and Fibonacci Levels')
                plt.xlabel('Time')
                plt.ylabel('Price')
                plt.legend()
                
                # Save plot to the datetime folder
                plot_filename = f"{token_address}_fib_plot.png"
                plot_path = os.path.join(datetime_folder, plot_filename)
...
async def analyze_qualifying_tokens(qualifying_tokens, max_cluster_percentage):
    """
    Analyzes qualifying tokens using bubblemap and other criteria.
    """
    results = []
...
async def run_analysis_cycle():
    try:
            ...
            async def read_stream(stream, is_error=False):
                while True:
                    ...
                    if not line:
                        break
                    if is_error:
                        logging.error(f"Token Generator Error: {line.strip()}")
                    else:
                        logging.info(f"Token Generator: {line.strip()}")
        ...
        # After fetching token data, create the folder structure
        new_token_data_folder = os.path.join(root_dir, 'Bots', 'Fib', 'Data', 'New_Token_Data')
        ...
        for folder in [new_token_data_folder, date_folder, ohlcv_folder, token_summary_folder]:
            os.makedirs(folder, exist_ok=True)
            logging.info(f"Created directory: {folder}")

        # Save the token data to token_data.csv in the Token_Summary folder
        token_data_path = os.path.join(token_summary_folder, 'token_data.csv')
        token_data_df.to_csv(token_data_path, index=False)  # Assuming token_data is a DataFrame

        logging.info(f"Token data saved to {token_data_path}")

        # Step 3: Perform Rug Check (Bubblemap Analysis)
        logging.info("Performing rug check (bubblemap analysis)...")
        
        # Initialize an empty list for tokens that pass the rug check
        rug_check_passed_tokens = []
        ...
        for token_address in token_addresses:
            ...
            if token_address in previously_qualified:
                logging.info(f"Skipping previously qualified token: {token_address}")
                continue
            
            # Perform rug check
            rug_check_results = await analyze_qualifying_tokens([{'token_address': token_address}], MAX_CLUSTER_PERCENTAGE)
            
            # If the token passed the rug check, add it to the list
            if rug_check_results:
                rug_check_passed_tokens.append(token_address)
                logging.info(f"Token {token_address} passed rug check")
            else:
                logging.info(f"Token {token_address} failed rug check")
        
        # Update token_addresses to only include tokens that passed the rug check
        token_addresses = rug_check_passed_tokens
        logging.info(f"Tokens passing rug check: {len(token_addresses)}")

        # Step 4: Import latest OHLCV data for the given token addresses
        logging.info("Importing latest OHLCV data for specified tokens...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Import OHLCV data
                imported_ohlcv_data = Master_Functions.import_ohlcv_data(ohlcv_datetime_folder, token_addresses)
        ...
        # Step 5: Copy token_data.csv to Token_Summary folder
        logging.info("Copying token_data.csv to Token_Summary folder...")
        
        # Get the most recent datetime folder in Token_Summary
        token_summary_datetime_folder = get_most_recent_folder(token_summary_folder)
        
        destination_path = os.path.join(token_summary_datetime_folder, 'token_data.csv')
        shutil.copy2(token_data_path, destination_path)
        logging.info(f"token_data.csv copied to {destination_path}")

        # Step 6: Initialize analysis parameters
        qualifying_tokens = []
        fib_levels = [0.236, 0.382, 0.5, 0.618, 0.786, 0.886]

        # Step 7: Run Fibonacci analysis using addresses from token_data.csv
        logging.info("Running Fibonacci analysis...")
        
        # Filter imported_ohlcv_data to only include tokens from token_data.csv
        filtered_ohlcv_data = {
            address: data for address, data in imported_ohlcv_data.items()
            if address in token_addresses
        }
        
        plot_price_and_fib_levels(
            imported_ohlcv_data=filtered_ohlcv_data,
        ...
        if qualifying_tokens:
            logging.info(f"Found {len(qualifying_tokens)} tokens passing all criteria")
            for result in qualifying_tokens:
